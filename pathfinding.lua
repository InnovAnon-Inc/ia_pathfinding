...
------ CHANGE: Added helper to convert star's flat path into a list of vectors for our movement logic
----local function format_star_path(flat_path)
----    if not flat_path or #flat_path == 0 then return nil end
----
----    local path = {}
----    -- star returns [x_dest, y_dest, z_dest, ..., x_start, y_start, z_start]
----    -- We process in reverse to get [start -> dest] and convert to vectors
...
----    end
----
----    -- Note: Usually, the first node is the mob's current position.
----    -- If we have more than 1 node, we can skip the first one to avoid "stuttering".
----    if #path > 1 then
----        table.remove(path, 1)
----    end
----
...
----    -- CHANGE: Using 'max_iterations' instead of 'max_iter' to match star.lua's expected key
----    return {
----        max_iterations = 1024,
----        max_jump = 1,
----        max_drop = 2,
...
----
----    -- CHANGE: Rounding coordinates to integers.
----    -- star.lua uses these as table keys; float keys would prevent finding neighbors.
...
----
----    -- CHANGE: star returns a flat table of numbers.
----    -- We must convert it to a list of vector objects and reverse it.
...
----    else
----        minetest.log('action', '[ia_pathfinding] No path found to ' .. minetest.pos_to_string(dest))
----        ia_pathfinding.clear_pathing_state(self)
----    end
----end
----
----function ia_pathfinding.process_pathfinding(self)
----    -- Cleanup if the object we were chasing is gone
----    if self._target_object and not self._target_object:get_pos() then
----        ia_pathfinding.clear_pathing_state(self)
----    end
----end
---- ia_pathfinding/pathfinding.lua
...
-- ia_pathfinding/pathfinding.lua
-- NOTE: handles virtual door passability for the 'star' pathfinder.
